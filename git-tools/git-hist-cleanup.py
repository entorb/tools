#!/usr/bin/env python3
"""Using git-filter-repo (via uvx) to cleanup git history."""
# ruff:noqa: S108 S603 S607

# run from git repo root dir
...
MY_REPO = Path(Path.cwd().name)

GITHUB_USER = "entorb"
CLONE_DIR = Path("/tmp/git-filter-repo-clone")
BACKUP_DIR = Path("/tmp/git-filter-repo-backup")
...
FILES_LOCK = [
    ...
    "pnpm-lock.yaml",
    "uv.lock",
    "streamlit-stats/uv.lock",  # entorb.net
]
...
def restore_and_commit(
    ...
    if restored_files:
        subprocess.run(["git", "add", *restored_files], check=True)
...
cmd = ["uvx", "git-filter-repo", "--prune-empty", "always", "--invert-paths"]
...
if DO_LOCK:
    restore_and_commit(FILES_LOCK, backup_repo_dir, "chore(deps): Lock")
if DO_PACKAGES:
    restore_and_commit(FILES_PACKAGES, backup_repo_dir, "chore(deps): Packages")
if DO_DOC:
    restore_and_commit(FILES_DOC, backup_repo_dir, "docs")
if DO_TOOLS:
    restore_and_commit(FILES_TOOLS, backup_repo_dir, "chore: Tools")
...
subprocess.run(["git", "push", "--force", "origin", "--all"], check=True)
subprocess.run(
...
# cspell:words knip perltidyrc prek reflog rumdl sonarcloud sonarlint
